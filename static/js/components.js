...
function songRow(song, i, opts = {}) {
  ...
  <div class="track group grid items-center gap-3 px-4 py-2 rounded hover:bg-white/10
              grid-cols-[20px_1fr_auto] ${showAlbum ? "md:grid-cols-[20px_4fr_3fr_minmax(120px,1fr)]" : "md:grid-cols-[20px_1fr_minmax(120px,1fr)]"}"
    ...
    <div class="flex items-center gap-3 min-w-0">
      ${cover}
      <div class="min-w-0">
        <p class="track-title font-medium truncate">${escapeHtml(song.title)}</p>
        <p class="text-sm text-neutral-400 truncate">
          <span data-action="open" data-href="#/artist/${song.artist.id}" class="hover:underline hover:text-white">${escapeHtml(song.artist.name)}</span>
        </p>
      </div>
    </div>
    ${albumCell}
    <div class="flex items-center gap-3 justify-end">
      ...
      <button data-action="more" data-song-id="${song.id}" class="more-btn opacity-0 group-hover:opacity-100 text-neutral-400 hover:text-white" title="More">${ICON.more}</button>
    </div>
  </div>`;
}
...
function mediaCard(item) {
  ...
  let title, subtitle;
  if (item.type === "album") { title = item.title; subtitle = item.artist.name + (item.year ? " · " + item.year : ""); }
  else if (item.type === "playlist") { title = item.name; subtitle = item.description || ("By " + (item.owner ? item.owner.name : "you")); }
  else { title = item.name; subtitle = "Artist"; }
...
function shelf(title, items, opts = {}) {
  ...
  <section class="mb-8">
    <div class="flex items-end justify-between mb-4">
      <h2 class="text-2xl font-bold hover:underline cursor-default">${escapeHtml(title)}</h2>
      ${link}
    </div>
...
function songToCard(song) {
  return {
    ...
    title: song.title, subtitle: song.artist.name, _song: song,
  };
}
...
function renderSearch(data, q) {
  ...
  if (nothing)
    return `<div class="view-enter px-6 pt-10 text-center">
      ...
      <p class="text-neutral-400">Please check your spelling or try different keywords.</p></div>`;
  ...
  const topResult = topSong ? `
    <div class="md:col-span-1">
      ...
      <div class="card p-5 group relative cursor-pointer" data-action="play-row" data-index="0">
        ...
        <p class="text-sm text-neutral-400 mt-1">Song · <span data-action="open" data-href="#/artist/${topSong.artist.id}" class="hover:underline">${escapeHtml(topSong.artist.name)}</span></p>
...
function renderAlbum(a) {
  ...
  <div class="view-enter">
    ${collectionHeader({
      ...
      subtitle: `<span data-action="open" data-href="#/artist/${a.artist.id}" class="font-bold hover:underline cursor-pointer">${escapeHtml(a.artist.name)}</span>`,
...
function renderPlaylist(p) {
  ...
  const editControls = p.editable ? `
    ...
    <button data-action="delete-playlist" class="text-neutral-400 hover:text-white" title="Delete playlist">
  ...
  <div class="view-enter">
    ...
    ${playAllBar(editControls)}
    <div class="px-2 pb-10">${songList(p.tracks, { showAlbum: true })}</div>
  </div>`;
}
  ...
  <div class="view-enter">
    <div class="page-head px-6 pt-16 pb-6 flex flex-col md:flex-row items-center md:items-end gap-6" style="--head:#444">
      ...
      <div class="text-center md:text-left">
        ...
        <p class="text-sm text-neutral-300">${fmtCount(a.monthly_listeners)} monthly listeners${a.genre ? " · " + escapeHtml(a.genre) : ""}</p>
      </div>
    </div>
    <div class="px-6 py-5 flex items-center gap-6">
...
function renderQueue() {
  const q = window.Player ? Player.queue : [];
  const idx = window.Player ? Player.index : -1;
  UI.viewTracks = q;
  UI.pageContext = null;
  if (!q.length)
    return `<div class="px-6 pt-10 text-center text-neutral-400"><p class="text-2xl font-bold text-white mb-2">Queue is empty</p>Play something to build your queue.</div>`;

  const now = idx >= 0 ? songRow(q[idx], idx, { showAlbum: true, showCover: true }) : "";
  const upcoming = q.map((s, i) => i > idx ? songRow(s, i, { showAlbum: true, showCover: true }) : "").join("");
  return `
  <div class="view-enter px-2 pt-6 pb-10">
    <h1 class="text-3xl font-bold px-4 mb-5">Queue</h1>
    ${now ? `<h2 class="text-lg font-bold px-4 mb-2">Now playing</h2>${now}` : ""}
    ${upcoming.trim() ? `<h2 class="text-lg font-bold px-4 mt-6 mb-2">Next up</h2>${upcoming}` : ""}
  </div>`;
}
